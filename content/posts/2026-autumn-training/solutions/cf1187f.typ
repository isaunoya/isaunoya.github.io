...
令 $A_j=E[binom(B,j)]$，即所有大小为 $j$ 的边集同时满足 $Y_i=1$ 的概率之和。展开 $B^k$ 时，每项至多涉及 $k$ 条不同的边；补充哨兵后共有 $n$ 条边，故只需 $j<=q=min(k,n)$（代码中边数 $m=n$）。由第二类 Stirling 数的恒等式得

$ E[B^k]=sum_(j=0)^q S(k,j)j!A_j. $

其中 $S(k,j)=S(k-1,j-1)+j S(k-1,j)$，$S(0,0)=1$，其余边界状态为 $0$。

将选中的边分解为极大连续段。不同段依赖的原变量集合不交，联合概率为各段概率之积。定义 $w_(l,t)$ 为边段 $[l,l+t)$ 全部满足 $Y_i=1$ 的概率，$D_(i,j)$ 为边前缀 $[0,i)$ 中所有大小为 $j$ 的边集的贡献之和，则 $A_j=D_(n,j)$。每段长度不超过总选边数 $j$，故 $w$ 也只需计算 $t<=q$。
